...
begin
	...
	IE_PP(ie) = clopset ("vimexam2")
	pp = IE_PP(ie)
	btype  = clgwrd ("vimexam2.boundary", Memc[boundary], SZ_BTYPE, BTYPES)
...
#	call ie_setboundary (im, col1, col2, line1, line2, btype, bconstant)
	...
	col1  = max (col1,  CX1(MI_CMG(im)))
	col2  = min (col2,  CX2(MI_CMG(im)))
	line1 = max (line1, CY1(MI_CMG(im)))
	line2 = min (line2, CY2(MI_CMG(im)))
...
begin
	...
	do i = 1, MI_NIMS(im) {
	    call imseti (MI_IM(im,i), IM_TYBNDRY, btypes[btype])
	    call imseti (MI_IM(im,i), IM_NBNDRYPIX, nbndrypix + 1)
	    if (btypes[btype] == BT_CONSTANT)
		call imsetr (MI_IM(im,i), IM_BNDRYPIXVAL, bconstant)
	}
end
