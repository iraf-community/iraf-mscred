...
begin
	...
	case ANGH:
	    ...
	    if (nscan() == 1) {
		call printf ("angh %g\n")
		    call pargr (clgetr ("simexam2.angh"))
	    } else {
		call clputr ("simexam2.angh", rval1)
		if (gtype == 's')
		    redraw = YES
	    }
	case ANGV:
	    ...
	    if (nscan() == 1) {
		call printf ("angv %g\n")
		    call pargr (clgetr ("simexam2.angv"))
	    } else {
		call clputr ("simexam2.angv", rval1)
		if (gtype == 's')
		    redraw = YES
	    }
	case BACKGROUND:
	...
	case BOUNDARY:
		...
		call clpstr ("vimexam2.boundary", Memc[cmd])
	case BOX:
	    switch (gtype) {
	    case 'c', 'u', 'j', 'k', 'l', 'r', 'v', 'e', 'h', '.':
	        call gargb (bval)
	...
	case BUFFER:
		...
		call printf ("buffer %g\n")
		    call pargr (clgetr ("rimexam2.buffer"))
	    } else {
		call clputr ("rimexam2.buffer", rval1)
		if (gtype == 'r' || gtype == '.')
		    redraw = YES
	    }
	case CEILING:
	...
	case CONSTANT:
		...
		call printf ("constant %g\n")
		    call pargr (clgetr ("vimexam2.constant"))
	    } else
		call clputr ("vimexam2.constant", rval1)
	case DASHPAT:
	    call gargi (ival)
	    if (nscan() == 1) {
		call printf ("dashpat %g\n")
		    call pargi (clgeti ("eimexam2.dashpat"))
	    } else {
		call clputi ("eimexam2.dashpat", ival)
		if (gtype == 'e')
		    redraw = YES
	    }
	case DEFKEY:
	...
	case EPARAM:
		...
		call gdeactivate (gp, 0)
	        switch (Memc[cmd]) {
	        case 'c':
		    call clcmdw ("eparam cimexam2")
	        case 'j':
		    call clcmdw ("eparam jimexam2")
	        case 'k':
		    call clcmdw ("eparam kimexam2")
	        case 'l':
		    call clcmdw ("eparam limexam2")
	        case 'r', '.':
		    call clcmdw ("eparam rimexam2")
	        case 's':
		    call clcmdw ("eparam simexam2")
	        case 'u', 'v':
		    call clcmdw ("eparam vimexam2")
	        case 'e':
		    call clcmdw ("eparam eimexam2")
	        case 'h':
		    call clcmdw ("eparam himexam2")
		}
		if (Memc[cmd] == gtype)
		    redraw = YES
	    }
	case FILL:
	    ...
	    if (nscan() == 1) {
		call printf ("fill %b\n")
		    call pargb (clgetb ("eimexam2.fill"))
	    } else {
		call clputb ("eimexam2.fill", bval)
		if (gtype == 'e')
		    redraw = YES
	    }
	case FLOOR:
	...
	case INTERVAL:
		...
		call printf ("interval %g\n")
		    call pargr (clgetr ("eimexam2.interval"))
	    } else {
		call clputr ("eimexam2.interval", rval1)
		if (gtype == 'e')
		    redraw = YES
	    }
	case LABEL:
	    call gargb (bval)
	    if (nscan() == 2) {
		call clputb ("eimexam2.label", bval)
		if (gtype == 'e')
		    redraw = YES
	    }
	...
	case MAGZERO:
		...
		call printf ("magzero %g\n")
		    call pargr (clgetr ("rimexam2.magzero"))
	    } else {
		call clputr ("rimexam2.magzero", rval1)
		if (gtype == 'r' || gtype == '.')
		    redraw = YES
	    }
	case AUTOREDRAW:
...
begin
	...
	case NCONTOURS:
		...
		call printf ("ncontours %g\n")
		    call pargi (clgeti ("eimexam2.ncontours"))
	    } else {
		call clputi ("eimexam2.ncontours", ival)
		if (gtype == 'e')
		    redraw = YES
	    }
	case NCSTAT:
	...
	case NHI:
	    ...
	    if (nscan() == 1) {
		call printf ("nhi %g\n")
		    call pargi (clgeti ("eimexam2.nhi"))
	    } else {
		call clputi ("eimexam2.nhi", ival)
		if (gtype == 'e')
		    redraw = YES
	    }
	case NLINES:
	...
	case RADIUS:
		...
		call printf ("radius %g\n")
		    call pargr (clgetr ("rimexam2.radius"))
	    } else {
		call clputr ("rimexam2.radius", rval1)
		if (gtype == 'r' || gtype == '.')
		    redraw = YES
	    }
	case ROUND:
...
begin
	...
	case YORDER:
		...
		call printf ("yorder %d\n")
		    call pargi (clgeti ("rimexam2.yorder"))
	    } else {
		call clputi ("rimexam2.yorder", ival)
		if (gtype == 'r' || gtype == '.')
		    redraw = YES
	    }
	case ZERO:
	    ...
	    if (nscan() == 1) {
		call printf ("zero %g\n")
		    call pargr (clgetr ("eimexam2.zero"))
	    } else {
		call clputr ("eimexam2.zero", rval1)
		if (gtype == 'e')
		    redraw = YES
	    }
	case UNLEARN:
	    ...
	    case 'c', 'u', 'j', 'k', 'l', 'r', 'v', 'e', 's', 'h', '.':
	        switch (Memc[cmd]) {
	        case 'c':
		    call clcmdw ("unlearn cimexam2")
	        case 'j':
		    call clcmdw ("unlearn jimexam2")
	        case 'k':
		    call clcmdw ("unlearn jimexam2")
	        case 'l':
		    call clcmdw ("unlearn limexam2")
	        case 'r', '.':
		    call clcmdw ("unlearn rimexam2")
	        case 's':
		    call clcmdw ("unlearn simexam2")
	        case 'u', 'v':
		    call clcmdw ("unlearn vimexam2")
	        case 'e':
		    call clcmdw ("unlearn eimexam2")
	        case 'h':
		    call clcmdw ("unlearn himexam2")
		}
		if (Memc[cmd] == gtype)
		    redraw = YES
	    default:
	...
	case NBINS:
	    ...
	    if (nscan() == 1) {
		call printf ("nbins %d\n")
		    call pargi (clgeti ("himexam2.nbins"))
	    } else {
		call clputi ("himexam2.nbins", ival)
		if (gtype == 'h')
		    redraw = YES
	    }
	case Z1:
	    ...
	    if (nscan() == 1) {
		call printf ("z1 %g\n")
		    call pargr (clgetr ("himexam2.z1"))
	    } else {
		call clputr ("himexam2.z1", rval1)
		if (gtype == 'h')
		    redraw = YES
	    }
	case Z2:
	    ...
	    if (nscan() == 1) {
		call printf ("z2 %g\n")
		    call pargr (clgetr ("himexam2.z2"))
	    } else {
		call clputr ("himexam2.z2", rval1)
		if (gtype == 'h')
		    redraw = YES
	    }
	case AUTOSCALE:
		...
		call printf ("autoscale %b\n")
		    call pargb (clgetb ("himexam2.autoscale"))
	    } else {
		call clputb ("himexam2.autoscale", bval)
		if (gtype == 'h')
		    redraw = YES
	    }
	case TOP_CLOSED:
		...
		call printf ("top_closed %b\n")
		    call pargb (clgetb ("himexam2.top_closed"))
	    } else {
		call clputb ("himexam2.top_closed", bval)
		if (gtype == 'h')
		    redraw = YES
	    }
	case ALLFRAMES:
	...
	case FITPLOT:
		...
		call printf ("fitplot %b\n")
		    call pargb (clgetb ("rimexam2.fitplot"))
	    } else {
		call clputb ("rimexam2.fitplot", bval)
		if (gtype == 'r')
		    redraw = YES
	    }
	case SIGMA:
	    ...
	    if (nscan() == 1) {
		call printf ("sigma %g\n")
		    call pargr (clgetr ("jimexam2.sigma"))
	    } else {
		call clputr ("jimexam2.sigma", rval1)
		if (gtype == 'j' || gtype == 'k')
		    redraw = YES
	    }
	case AXES:
	    call gargb (bval)
	    if (nscan() == 2) {
		call clputb ("simexam2.axes", bval)
		if (gtype == 's')
		    redraw = YES
	    }
	case FITTYPE:
