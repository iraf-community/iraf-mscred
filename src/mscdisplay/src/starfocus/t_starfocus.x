...
pointer	rng_open(), mimap()
errchk	mimap, open, imgetr, imgeti
...
begin
	...
	while (imtgetim (list, Memc[image], SZ_FNAME) != EOF) {
	    mi = mimap (Memc[image], READ_ONLY, 0)
	    im = MI_IM(mi,1)
	    call imseti (im, IM_TYBNDRY, TYBNDRY)
	    call imseti (im, IM_NBNDRYPIX, NBNDRYPIX)
	    ncols = CX2(MI_CMG(mi)) - CX1(MI_CMG(mi)) + 1
	    nlines = CY2(MI_CMG(mi)) - CY1(MI_CMG(mi)) + 1
	    nimages = nimages + 1
	    if (nimages == 1) {
		SF_NCOLS(sf) = ncols
		SF_NLINES(sf) = nlines
		...
		if (type == STARFOCUS) {
		    iferr (call mg_c2im (mi, wx, wy, im, xim, yim)) {
			call erract (EA_WARN)
			next
		    }
		    ip = 1
		    ...
		    # Set the steps and order evaluation.
		...
		do i = i1, i2, i3 {
		    ...
		    if (xim < SF_RADIUS(sf)-NBNDRYPIX ||
			xim > IM_LEN(im,1)-SF_RADIUS(sf)+NBNDRYPIX ||
		        yim < SF_RADIUS(sf)-NBNDRYPIX ||
			yim > IM_LEN(im,2)-SF_RADIUS(sf)+NBNDRYPIX)
			next
		    if (nexp == 1)
			j = nimages
		    else
...
begin
	...
	if (!xt_imnameeq (image, Memc[dname])) {
	    call sprintf (Memc[dname], SZ_LINE, "mscdisplay %s frame=%d")
		call pargstr (image)
		call pargi (frame)
