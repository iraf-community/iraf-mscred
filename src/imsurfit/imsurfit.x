...
real	b1x, b2x, b1y, b2y, mean
pointer	sp, x, y, fit, inbuf, outbuf, ranges

int	impnlr(), imgnlr()
real	ims_divzero(), asumr()
extern	ims_divzero
...
begin
	...
	if (MEDIAN(imfit) == YES)
	 ...
	 # Compute mean for residual output.
	 if (TYPE_OUTPUT(imfit) == RESID) {
	    mean = 0.
	    do i = 1, nlines {
		if (MEDIAN(imfit) == YES) {
		    Memr[y] = real (i)
		...
		call isvector (sf, Memr[x], Memr[y], Memr[fit], ncols)
		mean = mean + asumr (Memr[fit], ncols) / ncols
	    }
	    mean = mean / nlines
	    call imaddr (imout, "skymean", mean)
	}
	...
	do i = 1, nlines {
		...
		call aaddkr (Memr[outbuf], mean, Memr[outbuf], ncols)
	    case RESP:
	        call isvector (sf, Memr[x], Memr[y], Memr[outbuf], ncols)
