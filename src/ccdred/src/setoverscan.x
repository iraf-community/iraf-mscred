...
begin
	...
	type = clgwrd ("function", Memc[str], SZ_LINE, OVERSCAN_TYPES)
	if (READAXIS(ccd) == 1) {
	    ...
	    if (type < OVERSCAN_FIT) {
		call error (1,
		    "Overscan function type not allow with readaxis = 2")
	    } else {
		first = l1
		last = l2
		navg = last - first + 1
	...
	# Log the operation.
	if (type < OVERSCAN_FIT) {
	    call sprintf (BIASLOG(ccd), LEN_LOG,
		"Overscan is [%d:%d,%d:%d], function=%s")
		call pargi (c1)
		call pargi (c2)
		call pargi (l1)
		call pargi (l2)
	    ...
	    ovsnmean = asumr (Memr[overscan], npts) / npts
	    call sprintf (BIASLOG(ccd), LEN_LOG,
		"Overscan is [%d:%d,%d:%d], mean %g")
		call pargi (c1)
		call pargi (c2)
		call pargi (l1)
		call pargi (l2)
		call pargr (ovsnmean)
	    iferr (call imaddr (IN_IM(ccd), "OVSNMEAN", ovsnmean))
	        ;
	}
	call timelog (BIASLOG(ccd), LEN_LOG)
	call ccdlog (IN_IM(ccd), BIASLOG(ccd))
