...
begin
	...
	i = 1
	i = ctowrd (Memc[str1], i, Memc[subset1], sz_name)

	# A null subset string is ok.  If not null check for conflict
	# with previous subset IDs.
	if (Memc[str1] != EOS) {
	    call strcpy (Memc[subset1], Memc[subset3], sz_name)
		...
		while (fscan (fd) != EOF) {
			...
			j = log10 (real(i))
			Memc[subset3+sz_name-2+j] = EOS
			call sprintf (Memc[subset1], sz_name, "%s_%d")
