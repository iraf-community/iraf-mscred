...
pointer procedure xx_fps (fp, im, line, fd)
...
begin
	...
	return (xx_fpss (fp, im, line, col1, col2, line1, line2, fd))
end
...
pointer procedure xx_fpss (fp, im, line, col1, col2, line1, line2, fd)
...
begin
	...
	if (line >= FP_LMIN(fp) && line < FP_LMAX(fp)) {
		...
		if (Mems[bp+i] == FP_CVAL(fp)) {
			...
			if (line>FP_L1(fp,j) && line<FP_L2(fp,j)) {
				...
				data = xx_fpvals (fp, im, FP_L1(fp,j))
			    if (IS_INDEFS(Mems[FP_V2(fp,j)]))
				data = xx_fpvals (fp, im, FP_L2(fp,j))
			}
		    }
		}
	    }
	...
	data = FP_DATA(fp)
	call amovs (Mems[xx_fpvals(fp,im,line)], Mems[data], nc)
	j = 1
...
pointer procedure xx_fpvals (fp, im, line)
...
pointer procedure xx_fpi (fp, im, line, fd)
...
begin
	...
	return (xx_fpsi (fp, im, line, col1, col2, line1, line2, fd))
end
...
pointer procedure xx_fpsi (fp, im, line, col1, col2, line1, line2, fd)
...
begin
	...
	if (line >= FP_LMIN(fp) && line < FP_LMAX(fp)) {
		...
		if (Mems[bp+i] == FP_CVAL(fp)) {
			...
			if (line>FP_L1(fp,j) && line<FP_L2(fp,j)) {
				...
				data = xx_fpvali (fp, im, FP_L1(fp,j))
			    if (IS_INDEFI(Memi[FP_V2(fp,j)]))
				data = xx_fpvali (fp, im, FP_L2(fp,j))
			}
		    }
		}
	    }
	...
	data = FP_DATA(fp)
	call amovi (Memi[xx_fpvali(fp,im,line)], Memi[data], nc)
	j = 1
	for (c1=col1; c1<=col2 && Mems[bp+c1]==0; c1=c1+1)
	    ;
	while (c1 <= col2) {
...
pointer procedure xx_fpvali (fp, im, line)
...
pointer procedure xx_fpl (fp, im, line, fd)
...
begin
	...
	return (xx_fpsl (fp, im, line, col1, col2, line1, line2, fd))
end
...
pointer procedure xx_fpsl (fp, im, line, col1, col2, line1, line2, fd)
...
begin
	...
	if (line >= FP_LMIN(fp) && line < FP_LMAX(fp)) {
		...
		if (Mems[bp+i] == FP_CVAL(fp)) {
			...
			if (line>FP_L1(fp,j) && line<FP_L2(fp,j)) {
				...
				data = xx_fpvall (fp, im, FP_L1(fp,j))
			    if (IS_INDEFL(Meml[FP_V2(fp,j)]))
				data = xx_fpvall (fp, im, FP_L2(fp,j))
			}
		    }
		}
	    }
	...
	data = FP_DATA(fp)
	call amovl (Meml[xx_fpvall(fp,im,line)], Meml[data], nc)
	j = 1
	for (c1=col1; c1<=col2 && Mems[bp+c1]==0; c1=c1+1)
	    ;
	while (c1 <= col2) {
...
pointer procedure xx_fpvall (fp, im, line)
...
pointer procedure xx_fpr (fp, im, line, fd)
...
begin
	...
	return (xx_fpsr (fp, im, line, col1, col2, line1, line2, fd))
end
...
pointer procedure xx_fpsr (fp, im, line, col1, col2, line1, line2, fd)
...
begin
	...
	if (line >= FP_LMIN(fp) && line < FP_LMAX(fp)) {
		...
		if (Mems[bp+i] == FP_CVAL(fp)) {
			...
			if (line>FP_L1(fp,j) && line<FP_L2(fp,j)) {
				...
				data = xx_fpvalr (fp, im, FP_L1(fp,j))
			    if (IS_INDEFR(Memr[FP_V2(fp,j)]))
				data = xx_fpvalr (fp, im, FP_L2(fp,j))
			}
		    }
		}
	    }
	...
	data = FP_DATA(fp)
	call amovr (Memr[xx_fpvalr(fp,im,line)], Memr[data], nc)
	j = 1
	for (c1=col1; c1<=col2 && Mems[bp+c1]==0; c1=c1+1)
	    ;
	while (c1 <= col2) {
...
pointer procedure xx_fpvalr (fp, im, line)
...
pointer procedure xx_fpd (fp, im, line, fd)
...
begin
	...
	return (xx_fpsd (fp, im, line, col1, col2, line1, line2, fd))
end
...
pointer procedure xx_fpsd (fp, im, line, col1, col2, line1, line2, fd)
...
begin
	...
	if (line >= FP_LMIN(fp) && line < FP_LMAX(fp)) {
		...
		if (Mems[bp+i] == FP_CVAL(fp)) {
			...
			if (line>FP_L1(fp,j) && line<FP_L2(fp,j)) {
				...
				data = xx_fpvald (fp, im, FP_L1(fp,j))
			    if (IS_INDEFD(Memd[FP_V2(fp,j)]))
				data = xx_fpvald (fp, im, FP_L2(fp,j))
			}
		    }
		}
	    }
	...
	data = FP_DATA(fp)
	call amovd (Memd[xx_fpvald(fp,im,line)], Memd[data], nc)
	j = 1
	for (c1=col1; c1<=col2 && Mems[bp+c1]==0; c1=c1+1)
	    ;
	while (c1 <= col2) {
...
pointer procedure xx_fpvald (fp, im, line)
